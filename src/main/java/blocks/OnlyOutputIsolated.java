...
public class OnlyOutputIsolated {
	...
	public static void main(String argv[]) throws InterruptedException {
		...
		cepConfig.addMethodRef(OnlyOutputIsolated.class, ref);
		...
		cep = (EPServiceProviderSPI) EPServiceProviderManager.getProvider(
				OnlyOutputIsolated.class.getName(), cepConfig);
		...
		EPStatement res = isolatedService
				.getEPAdministrator()
				.createEPL(
						"@Name('RESULT')"
								+ "insert into RES "
								+ "select a_res=blocks.Main.factorial(b_res+1), current_timestamp as timestamp, "
		...
		isolatedService.getEPAdministrator().removeStatement(res);
