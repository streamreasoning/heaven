...
public class CSPARQLEngineFacade implements RSPEngine {
	...
	private class HeavenFormatter extends ResultFormatter {
		...
		public void update(Observable o, Object arg) {
			...
			for (RDFTuple t : r.getTuples()) {
				statementStrings = new TripleContainer(t.get(0), t.get(1), t.get(2));
				statements.add(statementStrings);				
			}
			long outputTimestamp = System.currentTimeMillis();
			next.process(new OutCTEvent("", statements, rspEventsNumber, 0, outputTimestamp, false));
			log.debug("Status[" + status + "] C-SPARQL has sent an OutCTEvent downstream");	
		}
	...
	public ExecutionState init() {
		...
		try {
			...
			log.debug("Status[" + status + "] An error occured the C-SPARQL Engine is not initialized");
			return status;
		}
	...
	public boolean process(CTEvent event) {
		...
		status = ExecutionState.READY;
		log.debug("Status[" + status + "] RSP event number"+ rspEventsNumber + "injected in C-SPARQL engine sending " + n + " triples");
