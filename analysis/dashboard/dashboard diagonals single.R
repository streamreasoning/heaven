...
plot_path <- function(exps, xlim, ylim){
  ...
  name <- "Da PAPER DASHBOARD SPLIT LOG_"
  ...
  for(i in 1:(length(exps)+1)){
      setEPS()
      postscript(paste(name,".eps", sep=""), onefile=TRUE) #inches
     ...
     #stmt inc
      ...
      title(paste(paste("D.a","EN",lat_report_rhodf$EN[INC_GRAPH],"EW", lat_report_rhodf$EW[NAIVE_GRAPH], "K", lat_report_rhodf$K[NAIVE_GRAPH]),"",  cex.lab=1.5, ylab="Memory (mb)", xlab="Latency(ms)"))
