...
public class RDFFileManager {
	...
	public static EventRepository buildRepoFromFile(int simSize) throws Exception {
		...
		// Map<String, EventDeclaration> edMap = new HashMap<String,
		// EventDeclaration>();
		...
		logger.info("Service repository created in: " + (t2 - t1) + " ms with " + er.getEds().size() + " EDs and " + er.getEps().size() + " EPs.");
		// for (EventDeclaration ed : er.getEds().values()) {
		// logger.info("ED: " + ed);
		// }
		// RDFDataMgr.write(System.out, dataset.getNamedModel(cesPrefix),
		// Lang.TURTLE);
	...
	public static Model createEDModel(Model m, EventDeclaration ed) throws NodeRemovalException {
		...
		// edID.addProperty(m.createProperty(prefixMap.get("rdf"), "type"),
		// m.createResource(ssnPrefix + "Sensor")); //
		// add profile
		edID.addProperty(m.createProperty(owlsPrefix + "presents"), profile);
		// for (String s : ed.getPayloads())
		// edID.addProperty(m.createProperty(ssnPrefix + "observes"),
		// m.createResource(s));
		...
		if (ed.getEp() == null) { // primitive event service
			...
			profile.addProperty(
					m.createProperty(owlsscPrefix + "serviceCategory"),
	...
	private static void createSelections(Resource patternNode, Model m, EventPattern ep) {
		...
		for (Selection sel : ep.getSelections()) {
			...
			// pName.addProperty(RDF.type,
			// pType).addProperty(m.createProperty(ssnPrefix + "isPropertyOf"),
			// foi);
			// }
		}
	}
	...
	private static void createQosParam(Resource profile, String pName, Object pValue, Model m) {
		profile.addProperty(
				...
				m.createResource()
						...
						.addProperty(m.createProperty(owlsspPrefix + "sParameter"),
								m.createResource().addLiteral(m.createProperty(emvoPrefix + "hasQuantityValue"), pValue)));
	}
	...
	private static Resource createTrafficLocation(Model m, EventDeclaration ed) {
		...
		firstNode.addLiteral(m.createProperty(ctPrefix + "hasStreetNumber"), ((TrafficReportService) ed).getNode1StreetNo());
		...
		secNode.addLiteral(m.createProperty(ctPrefix + "hasStreetNumber"), ((TrafficReportService) ed).getNode2StreetNo());
	...
	// private static void createPayloadModel(Resource profile, Model m,
	// EventDeclaration ed) {
	// Resource payload = m.createResource();
	// profile.addProperty(m.createProperty(cesPrefix, "hasEventPayload"),
	// payload);
	// for (String p : ed.getPayloads())
	// payload.addProperty(m.createProperty(ssnPrefix, "observedProperty"),
	// m.createResource(p));
	// Resource foi;
	// if (ed instanceof TrafficReportService) {
	...
	//
	// payload.addProperty(RDF.type, m.createResource(cesPrefix +
	// "EventPayload"))
	...
	public static void extractConstraintAndPreferenceById(String qid, QosVector constraint, WeightVector weight) {
		...
		while (constResults.hasNext()) {
			...
			RDFNode property = queryModel.getProperty((Resource) constId, queryModel.createProperty(cesPrefix + "onProperty")).getObject();
			// System.out.println("has property: " + property);
			RDFNode expression = queryModel.getProperty((Resource) constId, queryModel.createProperty(cesPrefix + "hasExpression")).getObject();
			...
			else if (queryModel.contains((Resource) expression, queryModel.createProperty(emvoPrefix + "lessOrEqualThan")))
			...
			else if (queryModel.contains((Resource) expression, queryModel.createProperty(emvoPrefix + "greaterOrEqualThan")))
		...
		while (prefResults.hasNext()) {
			...
			Double availabilityW = queryModel.getProperty((Resource) prefId, queryModel.createProperty(cesPrefix + "availabilityWeight")).getDouble();
			Double reliabilityW = queryModel.getProperty((Resource) prefId, queryModel.createProperty(cesPrefix + "reliabilityWeight")).getDouble();
			Double priceW = queryModel.getProperty((Resource) prefId, queryModel.createProperty(cesPrefix + "priceWeight")).getDouble();
			Double securityW = queryModel.getProperty((Resource) prefId, queryModel.createProperty(cesPrefix + "securityWeight")).getDouble();
			Double latencyW = queryModel.getProperty((Resource) prefId, queryModel.createProperty(cesPrefix + "latencyWeight")).getDouble();
	...
	private static EventDeclaration extractEDByServiceID(RDFNode edID, Dataset dataset, Map<String, EventDeclaration> edMap) {
		if (!edMap.containsKey(edID)) {
			...
			// System.out.println("Extracting: " + ed.toString() +
			// " serviceId: " + ed.getServiceId());
			return ed;
		}
		return edMap.get(edID);
	}

	// private static void createFilterForEventOperator(Resource bnode, Filter
	// f, Model m) {
	// Property hasFilter = m.createProperty(cesPrefix + "hasFilter");
	// Resource filter = m.createResource();
	// filter.addProperty(RDF.type, m.createResource(cesPrefix + "Filter"));
	// bnode.addProperty(hasFilter, filter);
	// filter.addProperty(m.createProperty(cesPrefix + "onPayload"),
	// m.createResource(f.getVar()));
	// Property op;
	// if (f.getOp() == Filter.eq)
	// op = m.createProperty(emvoPrefix + "equals");
	// else if (f.getOp() == Filter.lt)
	...
	private static Map<String, EventDeclaration> extractEDsFromDataset(Dataset dataset) {
		...
		String describeStr = queryPrefix + " select ?x"
	...
	private static String extractFoIById(Dataset dataset, String foiId) {
		String queryStr = queryPrefix + " select  ?a ?b ?c ?d where { <" + foiId + "> ct:hasStartLatitude ?a. <" + foiId
				+ "> ct:hasStartLongitude ?b. <" + foiId + "> ct:hasEndLatitude ?c. <" + foiId + "> ct:hasEndLongitude ?d.}";
		...
		if (results.hasNext()) {
			...
			// ed.setFoi(startLat + "," + startLon + "-" + endLat + "," +
			// endLon);
			// }
		}
		return null;
	...
	private static void extractEventFoIAsResource(EventDeclaration ed, Dataset dataset, String foiId) {
		...
		// extractTrafficLocation(ed, dataset);
		// } else {
		String queryStr = queryPrefix + " select  ?foi where { <" + ed.getServiceId() + "> ssn:observes ?p. ?p ssn:isPropertyOf ?foi.}";
		QueryExecution qe = QueryExecutionFactory.create(queryStr, dataset);
		ResultSet results = qe.execSelect();
		if (results.hasNext()) {
	...
	private static void extractEventPattern(EventDeclaration ed, Dataset dataset, Map<String, EventDeclaration> edMap)
		...
		String desribeStr = queryPrefix + " describe  ?z where {<" + ed.getnodeId() + "> owls:presents ?y. ?y ces:hasPattern ?z.}";
		...
		String queryRoot = queryPrefix + " select  ?z where {<" + ed.getnodeId() + "> owls:presents ?y. ?y ces:hasPattern ?z.}";
	...
	private static String extractEventPayloads(EventDeclaration ed, Dataset dataset) {
		String queryStr = queryPrefix + " select ?x ?y ?z where {<" + ed.getnodeId() + "> ssn:observes ?x. ?x rdf:type ?y. ?x ssn:isPropertyOf ?z }";
		QueryExecution qe = QueryExecutionFactory.create(queryStr, dataset);
		ResultSet results = qe.execSelect();
		...
		while (results.hasNext()) {
			...
			foiId = foi.toString();
			// System.out.println("payload: " + propertyType.toString() + "|" +
			// foi.toString() + "|"
	...
	private static void extractEventQoS(EventDeclaration ed, Dataset dataset) {
		...
		String queryStr = queryPrefix + " select  ?type ?value where {<" + ed.getnodeId()
	...
	private static void extractEventSource(EventDeclaration ed, Dataset dataset) {
		...
		String queryStr = queryPrefix + " select  ?z where {<" + ed.getnodeId() + "> owls:supports ?y. ?y ces:httpService ?z.}";
		QueryExecution qe = QueryExecutionFactory.create(queryStr, dataset);
		ResultSet results = qe.execSelect();
		if (results.hasNext()) {
	...
	private static Map<String, List<Filter>> extractFilterMap(String nodeId, List<RDFNode> filters, Model m, EventPattern ep) {
		...
		for (RDFNode filterNode : filters) {
			...
			RDFNode payload = m.getProperty((Resource) filterNode, m.createProperty(cesPrefix + "onPayload")).getObject();
			int op = 0;
			Object val;
			...
			if (m.getProperty((Resource) filterNode, m.createProperty(cesPrefix + "onEvent")) != null) {
				RDFNode eventId = m.getProperty((Resource) filterNode, m.createProperty(cesPrefix + "onEvent")).getObject();
				// if (eventId != null) {
	...
	public static EventPattern extractQueryFromModel(Model serviceRequest) {
		// Model queryBase = FileManager.get().loadModel(datasetDirectory +
		// serviceRequest);
	...
	private static void extractSelectionMap(RDFNode root, Model patternModel, EventPattern ep, Dataset dataset, Map<String, EventDeclaration> edMap)
			throws CloneNotSupportedException {

		List<RDFNode> selections = patternModel.listObjectsOfProperty((Resource) root, patternModel.createProperty(cesPrefix + "hasSelection"))
				.toList();
		if (selections.size() > 0) {
			ep.setSelections(new ArrayList<Selection>());
			for (RDFNode selection : selections) {
				RDFNode property = patternModel.getProperty((Resource) selection, patternModel.createProperty(cesPrefix + "selectedProperty"))
						.getObject();
				// System.out.println("extracting property: " +
				// property.toString());
				RDFNode foi = dataset.getDefaultModel()
						.listObjectsOfProperty((Resource) property, patternModel.createProperty(ssnPrefix + "isPropertyOf")).toList().get(0);
				List<RDFNode> types = dataset.getDefaultModel().listObjectsOfProperty((Resource) property, RDF.type).toList();
				String typeStr = "";
				...
				// }
				// }
				String nodeId = dataset.getDefaultModel().getProperty((Resource) selection, patternModel.createProperty(cesPrefix + "hasNodeId"))
						.getString();
				// RDFNode
				RDFNode serviceID = patternModel.createResource(((EventDeclaration) ep.getNodeById(nodeId)).getServiceId());
				EventDeclaration ed;
				if (ep.isQuery())
				...
				// ep.getSelectionMap().put(serviceID.toString(), new
				// ArrayList<String>());
				// }
	...
	private static void extractTrafficLocation(EventDeclaration ed, Dataset dataset) {
		...
		String queryStr = queryPrefix + " select ?z where { <" + ed.getnodeId() + "> ssn:observes ?y.   ?y ssn:isPropertyOf ?z.}  ";
		// +
		// " ?fn ct:hasNodeName ?fnn. ?fn ct:hasStreetNumber ?fnsn. ?fn ct:hasStreet ?fnst. ?fn ct:hasCity ?fnc. ?fn ct:hasLatitude ?fnlat. ?fn ct:hasLongtitude ?fnlon."
		// +
		// " ?sn ct:hasNodeName ?snn. ?sn ct:hasStreetNumber ?snsn. ?sn ct:hasStreet ?snst. ?sn ct:hasCity ?snc. ?sn ct:hasLatitude ?snlat. ?sn ct:hasLongtitude ?snlon.}";
		...
		if (results.hasNext()) {
			...
			// String firstNodeStreetNo =
			// row.get("fnsn").asLiteral().getString();
			...
			// String secondNodeStreetNo =
			// row.get("snsn").asLiteral().getString();
			// String secondNodeStreet =
			// row.get("snst").asLiteral().getString();
	...
	private static void loadOntology(Dataset dataset) {
		...
		// ssnBase));
		// FileManager.get().readModel(dataset.getNamedModel(owlPrefix),
		// ontologyDirectory + "Service.owl");
		//
		// FileManager.get().readModel(dataset.getNamedModel(owlPrefix),
		// ontologyDirectory + "Grounding.owl");
		// FileManager.get().readModel(dataset.getNamedModel(owlPrefix),
		// ontologyDirectory + "Process.owl");
		// FileManager.get().readModel(dataset.getNamedModel(owlPrefix),
		// ontologyDirectory + "Profile.owl");
		// FileManager.get().readModel(dataset.getNamedModel(cesPrefix),
		// ontologyDirectory + "ces.n3");
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// EventPattern query =
		// RDFFileManager.extractQueryFromDataset("SampleEventRequest.n3");
		// QosVector constraint = new QosVector();
		// WeightVector weight = new WeightVector();
		// RDFFileManager.extractConstraintAndPreferenceById(query.getID(),
		// constraint, weight);
		RDFFileManager.closeDataset();
	}

	private static void traverseToCreate(String rootId, Resource bnode, EventPattern ep, Model m) throws NodeRemovalException {
		Object node = ep.getNodeById(rootId);
		// Property hasSubPattern = m.createProperty(cesPrefix +
		// "hasSubPattern");
		if (node instanceof EventDeclaration) {
			bnode.addProperty(RDF.type, m.createResource(cesPrefix + "ServiceNode"));
			bnode.addProperty(m.createProperty(cesPrefix + "hasService"), m.createResource(((EventDeclaration) node).getServiceId()));
			...
			for (int i = 0; i < childIds.size(); i++) {
				...
				Resource cnode;// need to write rdf:Seq for sequence and
								// repetition
				// if (ep.getNodeById(cid) instanceof EventOperator ||
				// ep.hasFilterOn(cid))
	...
	private static String traverseToExtract(int eoCnt, RDFNode root, Model patternModel, EventPattern ep, Dataset dataset,
		...
		if (isOp) {// is an event operator
			...
			while (childNodes.hasNext()) {
				...
				// String selectStr = queryPrefix + " ask {<" + node.toString()
				// + "> rdf:type ces:ServiceNode.}";
				// QueryExecution qe = QueryExecutionFactory.create(selectStr,
				// dataset);
				if (typeList.contains(patternModel.getResource(cesPrefix + "ServiceNode"))) {// is
																								// an
																								// event
																								// service
																								// node
					...
					String nodeId = dataset.getDefaultModel().getProperty(node, hasNodeId).getObject().asLiteral().getString();
			...
			RDFNode service = patternModel.getProperty((Resource) serviceNode, patternModel.getProperty(cesPrefix + "hasService")).getObject();
			String serviceNodeId = patternModel.getProperty((Resource) serviceNode, patternModel.getProperty(cesPrefix + "hasNodeId")).getObject()
					.asLiteral().getString();
	...
	public static void writeEDsToFile(String fileName, List<EventDeclaration> eds) throws IOException, NodeRemovalException {
		Model serviceModel = ModelFactory.createDefaultModel();
		serviceModel.setNsPrefixes(prefixMap);
		int i = 0;
