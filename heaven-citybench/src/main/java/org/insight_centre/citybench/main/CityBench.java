...
public class CityBench {
	 ...
	 * @param prop
	 * @param parameters
	 *            Acceptable params: rates=(double)x, queryDuplicates=(int)y,
	 *            duration=(long)z, startDate=(date in the format of
	 *            "yyyy-MM-dd'T'HH:mm:ss")a, endDate=b, frequency=(double)c.
	...
	public CityBench(Properties prop, HashMap<String, String> parameters) throws Exception {
		...
		logger.info("Parameters loaded: engine - " + this.engine + ", queries - " + this.queries + ", rate - " + this.rate + ", frequency - "
				+ this.frequency + ", duration - " + this.duration + ", duplicates - " + this.queryDuplicates + ", start - " + this.start
				+ ", end - " + this.end);

		this.resultName = UUID.randomUUID() + " r=" + this.rate + ",f=" + this.frequency + ",dup=" + this.queryDuplicates + ",e=" + this.engine
				+ ",q=" + this.queries;// +
		// parameters.toString();
		// initialize datasets
		try {
	...
	/**
	 * parse the stream URIs in the queries and identify the stream file
	 * locations
	 * 
			...
			if (!queryDirectory.exists())
				...
				if (queryFiles != null) {
					for (File queryFile : queryFiles) {
						...
						String qStr = new String(Files.readAllBytes(java.nio.file.Paths.get(queryDirectory + File.separator + queryFile.getName())));
